  ...
  return (
    <div className="p-6 space-y-4">
      ...
      <div className="flex justify-between items-center">
        <Input placeholder="Tìm kiếm theo Mã phiếu hoặc Tên phiếu" className="w-96" onChange={(e) => handleSearchReceipts(e)} />
      ...
      {selectedReceipt && (<CapNhatNhapKhoDialog
        open={openEditDialog}
          ...
          <Table>
            ...
            <TableBody>
              {filteredReceipts.map((phieu) => {
                const tongGiaTri = phieu.chiTiet?.reduce(
                ...
                return (
                  <TableRow key={phieu.id}>
                    <TableCell>{dayjs(phieu.thoiGianTao).format('DD/MM/YYYY HH:mm')}</TableCell>
                    <TableCell>{phieu.maPhieu}</TableCell>
                    <TableCell>Nhập kho</TableCell>
                    <TableCell>{phieu.tenPhieu}</TableCell>
                    <TableCell>{phieu.nhaCungCap?.tenNhaCungCap || 'N/A'}</TableCell>
